...
from .utils import jira_api, salesforce_api, crowd_api, safe_cache
from commonweb.util.integration import makesafe


@safe_cache.call_safely
...
class Client(object):
    ...
    def __init__(self, id=None, is_new=False):
        ...
        if id not in (None, 'new', 'None'):
            record = db.clients.find_one({'_id': ObjectId(id)})
            if record is None:
                return
    ...
    def expire(self, key):
        ...
        if key in self.expire_helpers:
            ...
            if helper():
                db.clients.save(self._fields_dict())
    ...
    def get_jira_cases_with_extras(self, limit=10, jira_cases=None):
        ...
        # also get quarterly checkin tickets
