...
class Report(object):
    ...
    def get_next(self):
        ...
        # None if no run of this report has even
        # been scheduled
        curs = auxdb.reports.find({'name': self.name, 'status': {'$in': ['new', 'started']}})
    ...
    def start_one():
        ...
        try:
            run_record = auxdb.reports.find_and_modify(
                query={'status': 'new'},
        ...
        try:
            ...
            if run_record['notify']:
                ...
                if 'error' in run_record:
                    ...
                    body = link('clienthubviewreport', run_record['_id'])
...
class CountClients(Report):
    ...
    def run(self, run_record):
        ...
        for username in sorted(counts.keys()):
            ...
            rowdict['_link_' + fields[0]] = '%s?user=%s' % (link('clienthub'), username)
            rows.append(rowdict)
...
class ClientHubReports(app.page, CorpBase):
    ...
    def GET(self, PageParams):
        ...
        return env.get_template('clienthub/reports.html').render(
            reports=reports,
        )
...
class ClientHubScheduleReport(app.page, CorpBase):
    ...
    def GET(self, pageParams, reportname):
        ...
        web.header('Content-Type', 'text/html')
        return env.get_template('clienthub/report_schedule.html').render(
            report=report,
        )
...
class ClientHubViewReport(app.page, CorpBase):
    ...
    def GET(self, pageParams, report_run_id):
        ...
        web.header('Content-Type', 'text/html')
        return env.get_template('clienthub/report_view.html').render(
            report=report,
            result=result,
        )
